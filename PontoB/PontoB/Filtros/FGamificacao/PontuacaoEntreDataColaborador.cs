...
{
    public class PontuacaoEntreDataColaborador : IFiltro<Pontuacao>
    {
        ...
        public IList<Pontuacao> Filtrar(IQueryable<Pontuacao> query, string filtro)
        {
            ...
            if (DateTime.TryParse(valores.Fim.ToString(), out DateTime dataFim))
                return query.Include(c => c.Colaborador).OrderByDescending(r => r.DataRegistro).Where(e =>e.ColaboradorId.Equals(valores.Id) && e.DataRegistro >= valores.Inicio.Value.Date && e.DataRegistro < dataFim.Date.AddDays(1)).ToList();
